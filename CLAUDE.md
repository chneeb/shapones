...
**PSRAM SPI timing** (`psram_loader_init`): the system clock is **300 MHz** (`SYS_CLK_FREQ` in `common.hpp`), far above the polpo library's default clkdiv of 1.0. The current operating point is a **100 MHz PIO state-machine clock** (**50 MHz SCK**) with the **plain (non-fudge)** PIO program: the divisor is `SYS_CLK_FREQ / 100 MHz` (`psram_spi_init_clkdiv(pio1, -1, SYS_CLK_FREQ/(100*MHZ), false)`) → **3.0 @ 300 MHz**, 2.5 @ 250 MHz, with a `static_assert` guarding clocks that don't divide cleanly. Verified with 0 errors over a full 384 KB bulk read/verify.
