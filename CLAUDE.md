...
Planned work, with the evidence behind each item, is in `ROADMAP.md`. Findings
transcribed from the sibling InfoNES port are in `NOTES-from-infones.md`.
...
**Interlaced update** (`interlace_enabled` in `common.cpp`; toggle live with the **`i`** key). **Default is per-region**, set in `boot_nes()` once the ROM's region is known: **off for PAL/Dendy, on otherwise**. A full frame is 320×300×2 = 188 KB = 20.5 ms at the 75 MHz LCD SPI — hopeless against NTSC's 16.7 ms budget, but only 2.4% over PAL's 20.0 ms, so PAL runs progressive for about 1 fps and loses the combing entirely: a full-frame DMA is 320×300×2 = 192 KB — at ~75 MHz SPI that's ~20 ms, which was the frame-rate ceiling (~40 fps at the old 62.5 MHz SPI). With interlacing, each frame still converts all 300 rows but transfers only every *other* row (even/odd parity alternating via `lcd_field`); untouched rows keep the previous frame on the persistent panel. Each row is a 1-line async DMA that overlaps the next scanline's `cpu::service()`/conversion, so halving the transferred data roughly doubles the achievable rate — SMB3 now holds ~60 fps (the frame-pacing cap). The cost is faint interlace combing on fast vertical motion. `interlace_enabled` is a `volatile bool` flipped in `kbd_interrupt` (ISR) and latched once per frame in `cpu_loop` (`cur_interlace`) so a mid-frame toggle can't split a frame; the `else` branch is the original single full-frame path.
...
**Large ROMs (doesn't fit in heap)**: streamed into PSRAM via `psram_loader.cpp` (`psram_load_nes()`). The Picocalc has 8 MB PSRAM on GP20 (CS) / GP21 (SCK) / GP2 (MOSI) / GP3 (MISO), driven by the [polpo/rp2040-psram](https://github.com/polpo/rp2040-psram) library on **PIO1** (PIO0 is taken by the LCD). The library is a **vendored copy** at `samples/v3/rp2040-psram/` (a git submodule until 2026-09-21; see `VENDORED.md` there for provenance and why), wired into the build via `add_subdirectory` (it's an INTERFACE lib, so `psram_spi.c` compiles into `picocalc_nes` and picks up the `PSRAM_PIN_*` compile defs from CMakeLists). `psram_loader_init()` runs once in `main()` after `picocalc::init()`; it also resets loader state so a warm reboot into a different game starts clean.

**PSRAM SPI timing** (`psram_loader_init`): the system clock is **300 MHz** (`SYS_CLK_FREQ` in `common.hpp`), far above the polpo library's default clkdiv of 1.0. The current operating point is a **150 MHz PIO state-machine clock** (**75 MHz SCK**) in **single-bit SPI** — measured on this board at 5546 KB/s against 4097 KB/s at 50 MHz, both with zero errors. A QPI (quad) port is in progress on the **`psram-qpi`** branch — ~3.4x faster, working standalone but not yet in the loader; see `ROADMAP.md` §3 and `samples/v3/rp2040-psram/VENDORED.md`. What follows describes the SPI path on `main`, with the **plain (non-fudge)** PIO program: the divisor is `SYS_CLK_FREQ / 150 MHz` (`psram_spi_init_clkdiv(pio1, -1, SYS_CLK_FREQ/(150*MHZ), false)`) → **2.0 @ 300 MHz**, with a `static_assert` requiring an exact integer — the PIO divider is 16.8 fixed point and dithers the cycle length on a fractional value, which is fatal for this bus. Verified with 0 errors over a full 384 KB bulk read/verify.

**What governs reliability** is the choice of PIO program, which is *coupled to the clock* — not, as this note previously claimed, an oscillating sampling-phase window that fails at both fast and slow ends. `psram_spi.pio` documents the **fudge** variant's extra read-sync cycle as required **above 83 MHz SPI** and wrong below it. A sweep on the *same Picocalc PCB* with a byte-identical copy of the library (`~/Source/pico-286`, commits `c9c0458`/`7bd1691`/`d5aa195`) found the plain program passing **monotonically** at 49/66/79/99 MHz SPI; every slow-end failure was the fudge program used below its threshold. So there is no narrow window to fear — there is one two-state choice that must match the clock. **Getting that pairing wrong gives a dead bus**, and our call passes `false`, so raising the clock past 83 MHz SPI *must* flip it to `true`.

**We are running well below what this board does.** The same sweep soak-tested 99 MHz + fudge clean at ~5.0 MB/s against the ~2674 KB/s of the 49 MHz row we sit on — **1.9x** (freesci-archive, on an older fork of the driver, got only 1.25x for 1.49x the SPI rate and called it transaction-overhead-bound, so treat 1.25-1.9x as the honest range). Since a PRG cache miss is an 8 KB read on Core 0 inside `cpu::service()`, that is ~3.0 ms of a 16.6 ms frame today versus ~1.6 ms. `ROADMAP.md` §1 stages the upgrade (clkdiv 3.0 → 2.0 for 75 MHz is one line, an exact integer divisor and still plain-program). **Untested here as of 2026-09-18** — the numbers are inherited from that board, not measured on this firmware, so do **not** change the divisor without re-verifying against a full bulk read/verify; a 16-byte round-trip is too weak to catch marginal timing.

**`clk_peri` is not pinned.** The SDK's `set_sys_clock_pll()` ties `clk_peri` to `clk_sys` **undivided**, and nothing in `samples/v3/` or `samples/fatfs/` calls `clock_configure`. The LCD and PSRAM are PIO and carry their own clkdivs, but the **SD card is hardware `spi0`** (`samples/fatfs/source/mmc_pico_spi.c:127`), so it is clocked by whatever `SYS_CLK_FREQ` is. Harmless at 300 MHz (it works), but any sys-clock increase must pin `clk_peri` first or the SD returns corrupt reads — freesci-archive hit exactly this at 396 MHz (serial dead after the clock print, HardFault on load). See `ROADMAP.md` §1 step 2.

The build must define `PSRAM_ASYNC` in CMakeLists even though the loader only uses blocking transfers: the library's async inline helpers are unguarded and won't compile as C++ without it (freesci-archive gets away without it because its copy compiles as C).
...
**Known open bug**: Star Wars (NTSC, MMC3, 256 KB → PSRAM path) freezes early and at a different point each run. It does **not** reproduce in `samples/hosttest`, which maps the whole ROM directly, so the PRG bank cache is the leading suspect; see `ROADMAP.md` §8. Other MMC3-on-PSRAM games including SMB3 are fine, so it is a lead rather than a diagnosis.
