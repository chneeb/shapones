...
**PSRAM timing and mode** (`psram_loader_init`): the system clock is **300 MHz** (`SYS_CLK_FREQ` in `common.hpp`), far above the polpo library's default clkdiv of 1.0. The current operating point is a **100 MHz PIO state-machine clock** (**50 MHz SCK**) in **QPI (quad) mode** — `psram_enter_qpi()` brings the part up in SPI, sends Enter QPI (`0x35`), and re-inits with the quad program; chunk sizes become 127/123 bytes because the length field counts nibbles rather than bits in QPI. 75 MHz quad does **not** work on this board (see `ROADMAP.md` §3). Historically this ran single-bit SPI with the **plain (non-fudge)** PIO program: the divisor is `SYS_CLK_FREQ / 100 MHz` (`psram_spi_init_clkdiv(pio1, -1, SYS_CLK_FREQ/(100*MHZ), false)`) → **3.0 @ 300 MHz**, 2.5 @ 250 MHz, with a `static_assert` guarding clocks that don't divide cleanly. Verified with 0 errors over a full 384 KB bulk read/verify.
